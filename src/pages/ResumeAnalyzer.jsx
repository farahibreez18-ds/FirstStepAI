...
function ResumeAnalyzer() {
  ...
  const handleFile = async (e) => {
    ...
    try {
      ...
      setResult(data);
      await logActivity(currentUser?.uid, `Analyzed resume — scored ${data.score}/100`);
      await saveResumeScore(currentUser?.uid, data.score, data.verdict);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#0B1120] text-white relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 relative z-10">
        ...
        <label className="mt-5 flex flex-col items-center justify-center gap-3 border-2 border-dashed border-[#232D42] hover:border-[#4C6FFF] rounded-2xl py-14 cursor-pointer transition-colors bg-[#121A2E]/50">
        ...
        {result && (
          <div className="mt-10 grid md:grid-cols-2 gap-6">
            <div className="bg-[#121A2E] border border-[#232D42] rounded-2xl p-8 flex flex-col items-center justify-center">
              ...
              <p className="text-[#8A93A6] text-sm mt-5">{result.verdict}</p>
            </div>
